...
def collect_from_local_repo(repo_path: str, limit: int = 10) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for commit_line in commits:
            ...
            mrs.append(mr_data)
            logger.info(f"Collected MR: {title[:50]}... (complexity: {metrics['complexity_score']:.1f})")
...
def collect_from_artifacts(artifacts_dir: str) -> List[Dict[str, Any]]:
    ...
    for diff_file in diff_files:
        try:
            ...
            mrs.append(mr_data)
            logger.info(f"Collected MR from artifact: {diff_file.name} (complexity: {metrics['complexity_score']:.1f})")
