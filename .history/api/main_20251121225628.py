...
try:
    ...
    logger.warning(f"MR database module not available: {e}")
...
class ReviewerSessionCreate(BaseModel):
    ...
    mr_id: Optional[int] = None  # ID Merge Request для использования в сессии
...
def reviewer_create_session(payload: ReviewerSessionCreate):
    ...
    # Если указан mr_id, используем diff из MR вместо создания нового
    ...
    c.execute(
        ...
        (payload.candidate_name, payload.mr_package, json.dumps([]), now.isoformat() + 'Z', expires_at.isoformat() + 'Z', access_token, reviewer_token, payload.reviewer_name, 'active', candidate_id_safe, gitea_user if gitea_client else None, 0, mr_id)
    )
    session_id = c.lastrowid
    conn.commit()
    ...
    # Если указан MR, загружаем diff из него
    if mr_id:
        try:
            from mr_database import get_merge_request
            mr = get_merge_request(mr_id)
            if mr and mr.get('diff_content'):
                diff_path = f"/artifacts/{session_id}_diff.patch"
                artifacts_dir = "/artifacts"
                if not os.path.exists(artifacts_dir):
                    os.makedirs(artifacts_dir, exist_ok=True)
                with open(diff_path, 'w', encoding='utf-8') as f:
                    f.write(mr['diff_content'])
                logger.info(f"Loaded diff from MR {mr_id} for session {session_id}")
        except Exception as e:
            logger.warning(f"Failed to load diff from MR {mr_id}: {e}")
