...
def create_session(payload: SessionCreate):
    ...
    c.execute(
        ...
        (payload.candidate_id, payload.mr_package, json.dumps([]), now.isoformat(), expires_at.isoformat())
    )
...
def get_session(session_id: int):
    ...
    expires_at = row[5] if row[5] else (datetime.utcnow() + timedelta(hours=2)).isoformat()
...
async def upload_mr(file: UploadFile = File(...)):
    ...
    c.execute(
        "INSERT INTO sessions (candidate_id, mr_package, comments, created_at, expires_at) VALUES (?, ?, ?, ?, ?)",
        (f"upload_{int(time.time())}", file.filename, json.dumps([]), now.isoformat(), expires_at.isoformat())
    )
    session_id = c.lastrowid
    conn.commit()
    conn.close()
...
def extend_session(session_id: int):
    ...
    c.execute("UPDATE sessions SET expires_at = ? WHERE id = ?", (new_expires_at.isoformat(), session_id))
    conn.commit()
    conn.close()
