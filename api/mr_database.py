...
def init_mr_database():
    ...
    for migration_name in migrations:
        migration_file = os.path.join(os.path.dirname(__file__), "migrations", migration_name)
        ...
        try:
            ...
            # Используем autocommit для выполнения миграций
            # Это позволяет выполнять несколько команд последовательно
            pool = init_connection_pool()
            if pool is None:
                raise Exception("PostgreSQL connection pool not initialized")
            ...
            conn = pool.getconn()
            try:
                # Включаем autocommit для миграций
                old_autocommit = conn.autocommit
                conn.autocommit = True
                
                with conn.cursor() as cur:
                    # Выполняем весь SQL как есть
                    # psycopg2 может выполнять несколько команд через execute()
                ...
                conn.autocommit = old_autocommit
                # Игнорируем ошибки "column already exists", "trigger already exists" и подобные
                error_str = str(e).lower()
                if "already exists" in error_str or "duplicate" in error_str or "trigger" in error_str:
                    logger.info(f"Migration {migration_name} skipped (already applied)")
                else:
            ...
            with conn.cursor() as cur:
                cur.execute("""
                    INSERT INTO merge_requests (
                        ...
                        metadata, tags, mr_type, complexity_points, stack_tags
                    ) VALUES (
                        ...
                        %(metadata)s, %(tags)s, %(mr_type)s, %(complexity_points)s, %(stack_tags)s
                    ) RETURNING id
                """, {
                    ...
                    'mr_type': mr_data.get('mr_type'),
                    'complexity_points': mr_data.get('complexity_points', 3),
                    'stack_tags': mr_data.get('stack_tags', [])
                })
    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                ...
                if mr_type:
                    query += " AND mr_type = %s"
                    params.append(mr_type)
                
                if min_complexity_points is not None:
                    query += " AND complexity_points >= %s"
                    params.append(min_complexity_points)
                
                if max_complexity_points is not None:
                    query += " AND complexity_points <= %s"
                    params.append(max_complexity_points)
                
                if stack_tags:
                    # Поиск MR, у которых есть хотя бы один из указанных тегов
                    query += " AND ("
                    tag_conditions = []
                    for tag in stack_tags:
                        tag_conditions.append("%s = ANY(stack_tags)")
                        params.append(tag)
